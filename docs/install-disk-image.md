...
- User setup comes from `config.toml` (currently user `cin`, password `changeme`).
...
For non-Anaconda install paths (like raw/qcow2), time is set to UTC by default. Set timezone/geographic area after first boot from the Cinnamon tray clock time/date app.
